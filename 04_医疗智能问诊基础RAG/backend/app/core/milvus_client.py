...
for the medical knowledge vector database.
"""
...
def create_collection() -> Collection:
    """Create the medical_knowledge collection with schema and index."""
...
def delete_by_doc(doc_name: str) -> int:
    ...
    # Query to count before deletion
...
def list_docs() -> List[dict]:
    """
    List all unique documents with their chunk counts and creation times.
    Returns list of {doc_name, chunk_count, created_at}.
    """
    collection = get_collection()
    ...
    results = collection.query(
        expr="id >= 0",
        output_fields=["doc_name", "created_at"],
        limit=10000,
    )

    # Aggregate by doc_name
    doc_map = {}
    for row in results:
        name = row["doc_name"]
